...
int background_functions(
  ...
  if (pba->has_cdm == _TRUE_) {
    ...
    if (pba->has_dmde == _TRUE_){
      ...
      rho_tot += (1.-pba->frac_dmde)*pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3);
      rho_tot += f_scf(pba,pvecback_B[pba->index_bi_phi_scf])*pba->frac_dmde*pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3);

      rho_m += (1.-pba->frac_dmde)*pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3);
      rho_m +=f_scf(pba,pvecback_B[pba->index_bi_phi_scf])*pba->frac_dmde*pba->Omega0_cdm * pow(pba->H0,2) / pow(a_rel,3);
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    if (pba->has_dmde == _TRUE_) {
      //printf("index_bg_f_scf=%i\n",pba->index_bg_f_scf);
      pvecback[pba->index_bg_f_scf] = f_scf(pba,phi); //V_scf(pba,phi); //write here potential as function of phi
      pvecback[pba->index_bg_df_scf] = df_scf(pba,phi); // dV_scf(pba,phi); //potential' as function of phi
      pvecback[pba->index_bg_ddf_scf] = ddf_scf(pba,phi); // ddV_scf(pba,phi); //potential'' as function of phi
  ...
  if (pba->has_scf == _TRUE_){
    /** The contribution of scf was not added to dp_dloga, add p_scf_prime here: */ // TLS
    ...
    pvecback[pba->index_bg_p_prime_scf] = pvecback[pba->index_bg_phi_prime_scf]*
      (-pvecback[pba->index_bg_phi_prime_scf]*pvecback[pba->index_bg_H]/a-2./3.*pvecback[pba->index_bg_dV_scf]);
	    pvecback[pba->index_bg_p_prime_scf]=-pvecback[pba->index_bg_f_scf]*pvecback[pba->index_bg_rho_cdm]*pvecback[pba->index_bg_phi_prime_scf];
    pvecback[pba->index_bg_p_tot_prime] += pvecback[pba->index_bg_p_prime_scf];
  }
...
int background_free_input(
  ...
  if (pba->Omega0_scf != 0. && pba->has_dmde==_FALSE_){
...
int background_indices(
  ...
  class_define_index(pba->index_bg_f_scf,pba->has_dmde,index_bg,1);
  class_define_index(pba->index_bg_df_scf,pba->has_dmde,index_bg,1);
  class_define_index(pba->index_bg_ddf_scf,pba->has_dmde,index_bg,1);
...
int background_solve(
  ...
  double z_c_new, f_ede_new, phi_c_new;
  ...
  for (i=0; i < pba->bt_size; i++) {
    ...
    if(pba->has_scf== _TRUE_){
          ...
          f_ede_new = pvecback[pba->index_bg_rho_scf]/pvecback[pba->index_bg_rho_crit];
  ...
  if (pba->background_verbose > 2) {
    ...
    if (pba->has_scf == _TRUE_ && pba->has_dmde == _FALSE_){
    ...
    if (pba->has_dmde == _TRUE_){
     ...
     printf("     -> f_ede = %g and z_c = %g\n",
                    pba->f_ede,pow(10,pba->log10_z_c));
      if(pba->has_lambda == _TRUE_)
        printf("     -> Omega_Lambda = %g\n",
               pvecback[pba->index_bg_rho_lambda]/pvecback[pba->index_bg_rho_crit]);
...
int background_initial_conditions(
  ...
  */
   ...
   */
   ...
   if (pba->has_dmde == _TRUE_){
     //pba->phi_ini_scf = 0.;
     //pba->phi_prime_ini_scf = 3./2.*pba->frac_dmde*pba->Omega0_cdm*df_scf(pba,pba->phi_ini_scf)*pow(pba->H0,2)*pba->a_today/a/rho_rad;
     //pba->phi_prime_ini_scf =0.;
     pvecback_integration[pba->index_bi_phi_scf] = pba->phi_scf_ini;
     pvecback_integration[pba->index_bi_phi_prime_scf] = -0.5*(a/pba->a_today)/sqrt(rho_rad)*(dV_scf(pba,pba->phi_scf_ini)+
         df_scf(pba,pba->phi_scf_ini)*pba->frac_dmde*pow(pba->H0,2)*3.*pba->Omega0_cdm*pow(pba->a_today/a,3)); // Slow roll ICs
     // pvecback_integration[pba->index_bi_phi_prime_scf] = -0.5*df_scf(pba,0.0)*pba->frac_dmde*pba->Omega0_cdm
     // *pow(pba->H0,2)*pow(pba->a_today/a,2)/sqrt(rho_rad); // Slow roll ICs
     //printf("dphi_ini=%g\n",pvecback_integration[pba->index_bi_phi_prime_scf]);
     //pba->frac_dmde*pvecback_integration[pba->index_bg_rho_cdm]/rho_rad

    // *pba->Omega0_cdm*df_scf(pba,pba->phi_ini_scf)*pow(pba->H0,2)*pba->a_today/a/rho_rad;
...
int background_find_equality(
  ...
  while ((tau_plus - tau_minus) > ppr->tol_tau_eq) {
    ...
    //printf("tau=%g, Omega_m_over_Omega_r=%g\n",tau_mid,Omega_m_over_Omega_r);

    //printf("pvecback[pba->index_bg_a]=%g\n",pvecback[pba->index_bg_a]);
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"f_scf",pba->has_dmde);
  class_store_columntitle(titles,"f'_scf",pba->has_dmde);
  class_store_columntitle(titles,"f''_scf",pba->has_dmde);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_f_scf],pba->has_dmde,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_df_scf],pba->has_dmde,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_ddf_scf],pba->has_dmde,storeidx);
...
int background_derivs(
  ...
  if (pba->has_cdm){
    if (pba->has_dmde) {
      rho_M += (1.-pba->frac_dmde)*pvecback[pba->index_bg_rho_cdm];
      rho_M += f_scf(pba,pvecback[pba->index_bg_phi_scf])*pba->frac_dmde*pvecback[pba->index_bg_rho_cdm];
      //printf("%e\n",f_scf(pba,pvecback[pba->index_bg_phi_scf]));
    } else {
      rho_M += pvecback[pba->index_bg_rho_cdm];
      }
    }
  ...
  if (pba->has_scf == _TRUE_){
    /** - Scalar field equation: \f$ \phi'' + 2 a H \phi' + a^2 dV = 0 \f$  (note H is wrt cosmic time) Klein-Gordon*/
    ...
    dy[pba->index_bi_phi_prime_scf] = - y[pba->index_bi_a]*
      (2*pvecback[pba->index_bg_H]*y[pba->index_bi_phi_prime_scf]
       + y[pba->index_bi_a]*dV_scf(pba,y[pba->index_bi_phi_scf])+ y[pba->index_bi_a]*df_scf(pba,y[pba->index_bi_phi_scf])*
     pba->frac_dmde*3.*pvecback[pba->index_bg_rho_cdm]);
     // NOTE: in the EOM we need to couple to rho_c; in CLASS units rho_c,class = rho_c/(3 Mp**2) and when converting the field to be normalized
     // by Mp we get rho_c/Mp**2 -- therefore, we have 3*rho_c,class.
     //printf("Here! Potential is = %g\n",df_scf(pba,y[pba->index_bi_phi_scf]));
  }
...
double V_scf(
             struct background *pba,
             double phi) {
  //return  0.5*pow(pba->m_scf*phi,2);

  return  2225.30011*pow(pba->h,2)*pba->m_scf*pow(phi,4);
}

double dV_scf(
              struct background *pba,
              double phi) {
  //return pow(pba->m_scf,2)*phi;
  return  4.*2225.30011*pow(pba->h,2)*pba->m_scf*pow(phi,3);
}

double ddV_scf(
               struct background *pba,
               double phi) {
  //return pow(pba->m_scf,2);
  return 12.*2225.30011*pow(pba->h,2)*pba->m_scf*pow(phi,2);
}

double f_scf(
    struct background *pba,
    double phi)  {
 		 return exp(-pba->g_scf*phi);
}

double df_scf(
    struct background *pba,
    double phi)  {
 		 return -pba->g_scf*exp(-pba->g_scf*phi);
}

double ddf_scf(
    struct background *pba,
    double phi)  {
 		 return pow(pba->g_scf,2)*exp(-pba->g_scf*phi);
}
