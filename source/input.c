...
int input_init(
  ...
  char * const target_namestrings[] = {"100*theta_s","Omega_dcdmdr","omega_dcdmdr",
                                       "Omega_k","Omega_ini_dcdm","omega_ini_dcdm","sigma8"};
  char * const unknown_namestrings[] = {"h","Omega_ini_dcdm","Omega_ini_dcdm","Omega_Lambda",
  ...
  //printf("HERE!! unknown_parameters_size = %i\n",unknown_parameters_size);
  /**
...
int input_read_parameters(
  ...
  class_call(parser_read_double(pfc,"frac_dmde",&param1,&flag1,errmsg),
               errmsg,
               errmsg);

  class_call(parser_read_double(pfc,"g_scf",&param2,&flag2,errmsg),
               errmsg,
              errmsg);

  class_call(parser_read_double(pfc,"m_scf",&param3,&flag3,errmsg),
              errmsg,
              errmsg);

class_call(parser_read_double(pfc,"phi_scf_ini",&param4,&flag4,errmsg),
              errmsg,
              errmsg);

  // class_test(((flag1 == _FALSE_) || (flag2 == _FALSE_) || (flag3 == _FALSE_)),
  //                        errmsg,
  //                        "In input file, you need to enter all three dmde parameters");

  class_test(((flag1 == _TRUE_) && (param1>1) ),
              errmsg,
              "frac_dmde must be less than 1");

  class_test(((flag4 == _TRUE_) && (param4<0) ),
              errmsg,
              "phi_scf_ini must be positive");

  if (flag1 == _TRUE_){
    pba->frac_dmde = param1;
  ...
  //else if ((flag1 == _FALSE_) && (flag2 == _FALSE_) && (flag3 == _FALSE_)){
    // Fill up with scalar field
    //pba->Omega0_scf = 1. - pba->Omega0_k - Omega_tot;
    //if (input_verbose > 0) printf(" -> matched budget equations by adjusting Omega_scf = %e\n",pba->Omega0_scf);
  //}
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case Omega_k:
      ...
      output[i] = 1-ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_tot]/(ba.H0*ba.H0)-pfzw->target_value[i];
      break;
    case Omega_ini_dcdm:
    case omega_ini_dcdm:
      rho_dcdm_today = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm];
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_k:
      Omega_M = ba.Omega0_cdm+ba.Omega0_idm_dr+ba.Omega0_dcdmdr+ba.Omega0_b;
      xguess[index_guess]=1.-Omega_M-pfzw->target_value[index_guess];
      dxdy[index_guess] = -1.;
